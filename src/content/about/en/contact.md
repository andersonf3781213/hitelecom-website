---
{
  "section": "contact"
}
---
<Div class="clearfix nybanner">
	<Div class="nybanner_img" style=" background-image:url(/images/about/up_7428e98f9a91fcccdaa6c5ede77c3cc2.webp);">
	
	</Div>
	<Div class="nybanner_jj">
		<Div class="cell_mid">
			<div class="cell_h">
				<Div class="indexmid">
					<Div class="nybanner_jj_mid">
						<h1 class="nybanner_jj_mid_1 reveal"><img src="/images/about/static_images_img13.png" alt="Contact Hitelecom"/></h1>
						 
					</Div>
				</Div>
			</div>
		</Div>
	</Div>
	<DIV id="cate">
	
	</DIV>
</Div> 
<div class="clearfix pro_1 pro_1_ser" id="">
	<div class="indexmid">
		<Div class="pro_1_mid">
			<ul class="u5">
				 
				<li class="reveal">
					<a class="css3" href="/about">
						Company Overview
					</a>
				</li>
				<li class="reveal">
					<a class="css3" href="/about/quality">
						Quality &amp; Reliability
					</a>
				</li>
				<li class="reveal">
					<a class="css3" href="/about/partner">
						Partners
					</a>
				</li>
				<li class="reveal">
					<a class="css3" href="/about/joinus">
						Careers
					</a>
				</li>
				<li class="reveal">
					<a class="css3 on" href="/about/contact"> Contact Us </a>
				</li>
				 
			</ul>
			<select id="changeurl">
				 
				<option  value="/about">
					Company Overview
				</option> 
				<option     value="/about/quality">
					Quality &amp; Reliability
				</option> 
				<option    value="/about/partner">
					Partners
				</option> 
				<option    value="/about/joinus">
					Careers
				</option> 
				<option  selected="selected" value="/about/contact"> Contact Us </option> 
				
			</select>
		</Div>
	</div>
</div>
<div class="clearfix contact_1">
	<div class="indexmid">
		<div class="contact_1_mid">
			<div class="contact_1_mid_left">
				<div class="contact_1_mid_left_mid">
					<div class="clearfix contact_1_mid_left_mid_1 daxie reveal">
						Contact Us
					</div>
					<div class="clearfix contact_1_mid_left_mid_2 reveal">
						<ul>
							<li>
								+86-18616602589							</li>
							<li>
								2nd Floor, Building 1, No. 116 Cheyang Road, Songjiang District, Shanghai, China							</li>
							<li>
								sales@hitelecom.cn							</li>
						</ul>
					</div>
					<div class="clearfix contact_1_mid_left_mid_3 reveal">
						<p>
							<img width="104" alt="QR code for contacting Hitelecom" src="/images/about/static_1.png">
							<br>WhatsApp
						</p>
						<p>
							<img alt="QR code for contacting Hitelecom" src="/images/about/static_img53.png">
							<br>WeChat
						</p>
					</div>
				</div>
			</div>
			<div id="map_canvas">
			<img src="/images/about/contact-map.jpg" alt="Map of the Hitelecom office at No. 116 Cheyang Road, Songjiang District, Shanghai" loading="lazy" />
			<a class="map_more" href="https://www.openstreetmap.org/?mlat=30.9889&amp;mlon=121.2826#map=16/30.9889/121.2826" target="_blank" rel="noopener">View larger map</a>
			<span class="map_attr">© <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">OpenStreetMap</a> contributors</span>
			</div>
		</div>
	</div>
</div> 
<div class="clearfix pa_5 contact_2" id="feedback">
	<div class="indexmid">
							<Div class="index_1_1 textw reveal">
								Send an Inquiry
							</Div>
							<div class="index_1_2 textw daxie reveal" style="font-size:16px; letter-spacing:0">
								
							</div>
							<form id="form1" name="form1" method="post" onsubmit="return false">
							<div class="clearfix pa_5_1 reveal">
								
								<ul>
									<li style="display:none">
										 
									</li>
									<li>
										<div>
											<input type="text" id="name" name="name" placeholder="Name" value="">
										</div>
									</li>
									<li>
										<div>
											<input type="text" id="phone" name="phone" placeholder="Phone" value="">
										</div>
									</li>
									<li style="display:none">
										 
									</li>
									<li>
										<div>
											<input type="text" id="email" name="email" placeholder="Email" value="">
										</div>
									</li>
									<li style="display:none">
										 
									</li>
									<li>
										<div>
											<textarea id="message" name="message" placeholder="Tell us about your application, required quantity, connectivity, and deployment environment. We respond to most inquiries within one business day."></textarea>
										</div>
									</li>
								</ul>
							</div>
							<div class="clearfix pa_5_2 reveal">
								<a href="javascript:;" onClick="javascript:showAddr();">
									<span>Send Inquiry</span>
								</a>
							</div>
							<p class="privacy-consent">By submitting this form, you agree to our <a href="/about/privacy">Privacy Policy</a>.</p>
							</form>
	</div>
	 
</div> 
<Div class="clearfix daxie" id="footer">
