---
{
  "section": "quality"
}
---
<Div class="clearfix nybanner">
	<Div class="nybanner_img" style=" background-image:url(/images/about/up_7428e98f9a91fcccdaa6c5ede77c3cc2.webp);">
	
	</Div>
	<Div class="nybanner_jj">
		<Div class="cell_mid">
			<div class="cell_h">
				<Div class="indexmid">
					<Div class="nybanner_jj_mid">
						<h1 class="nybanner_jj_mid_1 reveal"><img src="/images/about/static_img13.png" alt="宏太云 · 物联 · 传感"/></h1>
						 
					</Div>
				</Div>
			</div>
		</Div>
	</Div>
	<div id="cate">
	
	</div>
</Div> 
<div class="clearfix pro_1 pro_1_ser" >
	<div class="indexmid">
		<Div class="pro_1_mid">
			<ul class="u5">
				 
				<li class="reveal">
					<a class="css3" href="/zh/about">
						企业简介
					</a>
				</li>
				<li class="reveal">
					<a class="css3 on" href="/zh/about/quality">
						质量与可靠
					</a>
				</li>
				<li class="reveal">
					<a class="css3" href="/zh/about/partner">
						生态伙伴
					</a>
				</li>
				<li class="reveal">
					<a class="css3" href="/zh/about/joinus">
						人才招聘
					</a>
				</li>
				<li class="reveal">
					<a class="css3" href="/zh/about/contact">
						联系我们
					</a>
				</li>
				 
			</ul>
			<select id="changeurl">
				 
				<option  value="/zh/about">
					企业简介
				</option> 
				<option  selected="selected"    value="/zh/about/quality">
					质量与可靠
				</option> 
				<option    value="/zh/about/partner">
					生态伙伴
				</option> 
				<option   value="/zh/about/joinus">
					人才招聘
				</option> 
				<option   value="/zh/about/contact">
					联系我们
				</option> 
				
			</select>
		</Div>
	</div>
</div>
<div class="clearfix qa_1">
	<div class="indexmid">
		<div class="clearfix qa_1_1">
			<div class="qa_1_1_left">
							<Div class="index_1_1 qa_1_1_1 reveal">
								质量方针
							</Div>
							<div class="index_1_2 qa_1_1_2 daxie reveal">
								Quality policy
							</div>
			</div>
			<div class="qa_1_1_right reveal">
				<p class="p1">精心设计，精益生产，提供超越期待的高质量产品和服务</p><p>在宏太的电子制造工厂，每一步生产流程都经过精心设计和严格监控，以确保我们的产品能够达到甚至超过行业标准。六西格玛30PPM内高质量追求，建立持续改进的质量管理体系，通过定期审查和优化工艺流程，最终实现卓越质量的目标。<br/><br/></p>			</div>
		</div>
		<div class="clearfix qa_1_2 reveal">
			<img alt="工程师在实验室检测 PCB 板" src="/images/about/up_33f8ecc7bf8fbfcf1538a9c87e898b51.webp">
		</div>
	</div>
</div> 
<div class="clearfix qa_2">
	<Div class="indexmid">
		<Div class="qa_2_mid">
							<Div class="index_1_1 qa_1_1_1 textw reveal">
								可靠性
							</Div>
							<div class="index_1_2 qa_1_1_2 textw daxie reveal">
								Reliability
							</div>
							<Div class="qa_2_mid_1 reveal">
								<p>在我们自研的产品中，可靠性不仅是一个承诺，而是我们设计和制造每一件产品或部件的核心原则。<br/>我们深知，物联设备经久耐用和长期稳定性对客人来说至关重要，因些我们采用前沿的物联网与人工智能技术，
精选的高品质材料到精细的工艺流程，确保设备在极端温度和湿度条件下也能保持性能不变。<br/>且宏太长期与国内外上市的芯片级实验室合作，为其定制设计老化测试板。模拟产品在极高的温度湿气及高压环境下，
连续通电试验检测产品本身功能特性，评估产品的超长时间的操作寿命及结构对湿气抵抗能力。<br/>深入理解产品可靠性要求，为自研设计团队提供有力帮助及支撑，宏太每一款产品在出厂前都要经过严格的老化测试
到的各种挑战，确保其高度的稳定性。为客人提供稳定可靠的智能体验，助力工业场景更加便捷、高效。</p>
							</Div>
		</Div>
	</Div>
</div>
<div class="clearfix qa_3">
	<Div class="indexmid">
							<Div class="index_1_1 reveal">
								宏太专业的设计和生产团队
							</Div>
							<div class="index_1_2 daxie reveal" id="qa_3_1" style="color:#666;">
								我们的研发和制造团队共同致力于为客户，行业，社会提供创新节能的物联网产品
							</div>
							<Div class="clearfix qa_3_1 reveal">
								<ul>
																		<li>
										<div class="qa_3_1_mid">
											<Div class="qa_3_1_mid_1">
												<img alt="技术人员在 PCB 板上焊接元件" src="/images/about/up_d94a0df42445c6fbcf77d20de0969c73.webp">
											</Div>
											<div class="qa_3_1_mid_jj_1 css3">
												<div class="qa_3_1_mid_jj">
													<Div class="qa_3_1_mid_jj_1_name">
														研发中心													</Div>
													<div class="qa_3_1_mid_jj_2_ename">
														R&D Center 													</div>
												</div>
											</div>
											<div class="css3 qa_3_1_mid_jj_1 qa_3_1_mid_jj_1_po">
												<div class="qa_3_1_mid_jj">
													<Div class="qa_3_1_mid_jj_1_name">
														研发中心													</Div>
													<div class="qa_3_1_mid_jj_2_ename">
														R&D Center 													</div>
													<div class="qa_3_1_mid_jj_2_jj">
														<div class="clearfix c1">宏太始终以客户为中心，坚持“极简、易用、可靠”的产品设计理念，深入考量用户的实际需求和使用环境，推出了风格独特、用户体验卓越的产品系列。宏太一直坚持品牌原创凭借不断的创新和研发已获得多项专利权。</div>
<div class="clearfix c2"><p>强大的研发团队</p><p>聚焦物联网技术，软硬件定制开发</p><p>提供公有和私有及混合云服务和定制</p><p>不断推出创新而实用的物联网整体解决方案</p></div>
													</div>
												</div>
											</div>
										</div>
									</li>
																		<li>
										<div class="qa_3_1_mid">
											<Div class="qa_3_1_mid_1">
												<img alt="SMT 贴片机生产线" src="/images/about/up_0ff68ee5ee21bb828e5ba0dd7b20ff56.webp">
											</Div>
											<div class="qa_3_1_mid_jj_1 css3">
												<div class="qa_3_1_mid_jj">
													<Div class="qa_3_1_mid_jj_1_name">
														生产中心													</Div>
													<div class="qa_3_1_mid_jj_2_ename">
														Production Center 													</div>
												</div>
											</div>
											<div class="css3 qa_3_1_mid_jj_1 qa_3_1_mid_jj_1_po">
												<div class="qa_3_1_mid_jj">
													<Div class="qa_3_1_mid_jj_1_name">
														生产中心													</Div>
													<div class="qa_3_1_mid_jj_2_ename">
														Production Center 													</div>
													<div class="qa_3_1_mid_jj_2_jj">
														<div class="clearfix c1">我们拥有一个经验丰富的制造团队，其核心技术人员均出身于国内知名大厂如富士康和广达，他们精通先进的电子贴装技术和装配流程。这不仅确保了我们设计的可实施性，也大大提高了生产效率，实现了成本的有效控制和优化。为客户提供更高质量、更经济高效的产品解决方案。
</div>
<div class="clearfix c2"><p>经验丰富的制造团队</p><p>技术工程人员来自国内大厂富士康广达</p><p>精通先进的电子贴装制造及装配技术和流程</p><p>确保设计可实施性和生产的高效性不断优化提效降本</p></div>
													</div>
												</div>
											</div>
										</div>
									</li>
																	</ul>
							</Div>
	</Div>
</div>
<div class="clearfix qa_4">
	<Div class="indexmid">
							<Div class="index_1_1 reveal">
								车规级生产贴装设备
							</Div>
							<div class="index_1_2 daxie reveal" id="qa_3_1">
								<div class="qa_7_font" style="max-width:1147px;">
									技术领先的硬件平台和强大的精密设备是质量构建基础，宏太电子采用前沿先进生产设备+检测设备，高品质保障并获得多数客人认可。宏太高精度贴装设备能够实现极其精准的元件放置，保证了电路板上每个微小元件（01005,BGA Pitch:0.35mm）的正确位置和最佳性能。
									与此同时，我们引入的先进检测设备，如锡膏检测(3D SPI)、自动光学检测(AOI)、X射线检测系统和功能测试设备，能够在生产过程中及时发现并纠正任何潜在的缺陷。对于满足复杂电子产品的高标准要求至关重要，确保产品的一致性和可靠性。



								</div>
							</div>
							<Div class="clearfix qa_4_2 reveal">
								<ul>
																		<li>
										<div class="qa_4_2_mid">
											<div class="qa_4_2_mid_1">
												<div class="cell_mid">
													<div  class="cell_h">
														<img alt="MPM 锡膏印刷机" src="/images/about/up_1551d73dc647f7ef7f4c034c70f06bd3.jpg">
													</div>
												</div>
											</div>
											<div class="qa_4_2_mid_2">
												<div class="qa_4_2_mid_2_mid">
													<div class="qa_4_2_mid_2_mid_1">
														MPM 印刷机 													</div>
													<div class="qa_4_2_mid_2_mid_2">
														Momentum BTB系列													</div>
												</div>
											</div>
										</div>
									</li>
																		<li>
										<div class="qa_4_2_mid">
											<div class="qa_4_2_mid_1">
												<div class="cell_mid">
													<div  class="cell_h">
														<img alt="Koh Young KY8030 锡膏检测设备" src="/images/about/up_b691ba179e8b6fd9f50bf5e838eaa38f.jpg">
													</div>
												</div>
											</div>
											<div class="qa_4_2_mid_2">
												<div class="qa_4_2_mid_2_mid">
													<div class="qa_4_2_mid_2_mid_1">
														Koh Young SPI 													</div>
													<div class="qa_4_2_mid_2_mid_2">
														三维锡膏在线检测系统													</div>
												</div>
											</div>
										</div>
									</li>
																		<li>
										<div class="qa_4_2_mid">
											<div class="qa_4_2_mid_1">
												<div class="cell_mid">
													<div  class="cell_h">
														<img alt="Koh Young 3D 锡膏检测（SPI）设备" src="/images/about/up_5a4e0e074312dca7403899d7b15cb71b.jpg">
													</div>
												</div>
											</div>
											<div class="qa_4_2_mid_2">
												<div class="qa_4_2_mid_2_mid">
													<div class="qa_4_2_mid_2_mid_1">
														OMRON AOI													</div>
													<div class="qa_4_2_mid_2_mid_2">
														VS 500系列													</div>
												</div>
											</div>
										</div>
									</li>
																		<li>
										<div class="qa_4_2_mid">
											<div class="qa_4_2_mid_1">
												<div class="cell_mid">
													<div  class="cell_h">
														<img alt="Rehm 回流焊炉" src="/images/about/up_468621c1477a5202b83424859b0fca9c.webp">
													</div>
												</div>
											</div>
											<div class="qa_4_2_mid_2">
												<div class="qa_4_2_mid_2_mid">
													<div class="qa_4_2_mid_2_mid_1">
														REHM Reflow<br>回流焊													</div>
													<div class="qa_4_2_mid_2_mid_2">
														REHM Reflow VS系列													</div>
												</div>
											</div>
										</div>
									</li>
																		<li>
										<div class="qa_4_2_mid">
											<div class="qa_4_2_mid_1">
												<div class="cell_mid">
													<div  class="cell_h">
														<img alt="高速 SMT 贴片机组" src="/images/about/up_467ee1a9f302751c62c181b898b4b25f.webp">
													</div>
												</div>
											</div>
											<div class="qa_4_2_mid_2">
												<div class="qa_4_2_mid_2_mid">
													<div class="qa_4_2_mid_2_mid_1">
														Panasonic 													</div>
													<div class="qa_4_2_mid_2_mid_2">
														NPM-W2 贴片机													</div>
												</div>
											</div>
										</div>
									</li>
																	</ul>
							</Div>
	</Div>
</div>
<div class="clearfix index_2 index_5 qa_5">
	 
	<Div class="indexmid">
		<Div class="index_1_1 reveal">
			精选高品质原辅材料 
		</Div>
		<div class="index_1_2 reveal">
			<div class="qa_7_font" style="max-width:936px;">
				宏太与全球范围内知名的芯片制造商和原辅材料供应商建立长期合作关系。与供应链紧密协作，共同提升原材料和组件的质量
确保产品从源头到出厂都符合高标准。

			</div>
		</div>
		<div class="clearfix index_5_1 reveal">
			
			<div class="swiper-container swiper-container-bannerp">
				<div class="swiper-wrapper">
 				 
				  				  <div class="swiper-slide"  >
				  	   <ul>
					   	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="GigaDevice（兆易创新）——宏太通信元件合作伙伴" src="/images/about/up_5cd3315cc09f1f18cc8fae819f11c81b.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="STMicroelectronics（意法半导体）——宏太通信元件合作伙伴" src="/images/about/up_42118537ac56b8ded21d336cd41e1cab.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="NXP（恩智浦）——宏太通信元件合作伙伴" src="/images/about/up_56547d4554c86773bd7946e5c62075c1.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="HDSC（华大半导体）——宏太通信元件合作伙伴" src="/images/about/up_614df29f7b1c439cccaa3ab35ab3e8e6.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="Renesas（瑞萨电子）——宏太通信元件合作伙伴" src="/images/about/up_8c93c59f532fa89a50e29599d77608e6.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="TDK——宏太通信元件合作伙伴" src="/images/about/up_e1e7a1a741afc8af0da065dcab01d140.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="muRata（村田制作所）——宏太通信电子元件合作伙伴" src="/images/about/up_0f2e973fee69307b9b26f6cf0a05ebd4.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="Analog Devices（ADI）——宏太通信元件合作伙伴" src="/images/about/up_2038498b1f33bdd16309af26916f7a9d.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="Texas Instruments（德州仪器）——宏太通信元件合作伙伴" src="/images/about/up_d7f7d000d24767ce52f09e62f8d9fb5a.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="Microchip（微芯科技）——宏太通信元件合作伙伴" src="/images/about/up_95fb54fc5097ead2bf72490771bab09d.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="SMIC（中芯国际）——宏太通信元件合作伙伴" src="/images/about/up_5e50988b00a1422e4c1c620f4040e3a8.jpg">
								</div>
							</div>
						</li>
						  	
					   	<li class="">
							<div class="cell_mid">
								<div class="cell_h">
									<img alt="Sensirion（盛思锐）——宏太通信传感器合作伙伴" src="/images/about/up_2e497c9d8d4ff112f29ea913981fa894.jpg">
								</div>
							</div>
						</li>
						  					   </ul>
				 </div>
				 				
				
				 </div>
				<div class="swiper-pagination swiper-pagination-banner2" id="swiper-pagination-banner2"></div>
	        </div>
			
		</div>
	</Div>
</div>
<div class="clearfix qa_6">
	<div class="indexmid">
		<Div class="qa_6_mid">
			<div class="qa_6_mid_left">
				<img alt="宏太通信质量管理体系证书与产品合格证" src="/images/about/up_f69d9bcdfa0aae6fa841fef95467c1f3.webp">
				<p class="cert_caption reveal">ISO 9001：NOA2302368 · ISO 14001：NOA2212190 · ISO 45001：NOA2212189<br>签发机构：挪亚检测认证集团有限公司</p>
			</div>
			<div class="qa_6_mid_right">
				<div class="qa_6_mid_right_1 reveal">
					严格的质量控制流程
				</div>
				<div class="qa_6_mid_right_2 reveal">
					<p>宏太团队采用全面质量管理（TQM）的管理理念<br/></p><p>确保产品开发、生产、销售和服务的每一步都进行质量控制</p><p>通过全面参与和持续改进，目标是实现零缺陷的产品和服务</p><p>我们已通过ISO 9001质量管理体系认证；此前曾于2019年9月19日至2025年9月18日通过IATF 16949:2016汽车行业质量管理体系认证（签发机构：TÜV NORD CERT GmbH，证书编号：NO.44111191175，IATF注册号：0359402）</p>				</div>
			</div>
		</Div>
	</div>
</div>
<div class="clearfix index_2 index_5 qa_7">
	 
	<Div class="indexmid">
		<Div class="index_1_1 reveal">
			宏太的绿色之旅 
		</Div>
		<div class="index_1_2 reveal">
			<div class="qa_7_font">
				 宏太作为现代制造业的一分子，我们对绿色地球可持续发展和人类健康肩负责任。正因如此，我们不仅遵守RoHS合规性、EU REACH法规和无卤要求，更通过ISO 14001环境管理体系的认证，将环境保护的承诺融入我们的日常运营和长远规划之中。


			</div>
		</div>
		<div class="clearfix qa_7_1 reveal">
			<div class="qa_7_1_left">
				
				<div class="swiper-container swiper-container-banner2d">
				<div class="swiper-wrapper">
 				  					 
				  <div class="swiper-slide" lang="0">
				  	<div class="cjimg">
						<img alt="仰望树冠与天空" src="/images/about/up_4343f2f6b506c7d16d7eb8a25d9d3569.webp">
					</div>
				   		 <Div class="qa_7_1_left_1">
						 	RoHS合规性						 </Div>
						 <div class="qa_7_1_left_2">
						 	RoHS（Restriction of Hazardous Substances Directive）全称是《关于限制在电子电气设备中使用某些有害成分的指令》，限制在电气和电子设备中使用四种重金属（铅、汞、镉、六价铬）和两种溴化物阻燃剂（多溴联苯PBB和多溴二苯醚PBDE）等，宏太提供给客户的产品符合RoHS标准。



						 </div>
				  </div>
				  					 
				  <div class="swiper-slide" lang="1">
				  	<div class="cjimg">
						<img alt="绿色星球——环保理念" src="/images/about/up_7d0bfe246419c5d58653bbe289701260.webp">
					</div>
				   		 <Div class="qa_7_1_left_1">
						 	EU REACH						 </Div>
						 <div class="qa_7_1_left_2">
						 	REACH全称为《Regulation Concerning the Registration, Evaluation，Authorization and Restriction of Chemicals》，即《化学品的注册、评估、授权和限制》，是欧盟对进入其市场的所有化学品进行预防性管理的法规，适用于所有化学品的生产、进口和使用。宏太提供给客户产品符合REACH认证。最新的化学物质限制请联系我们销售人员及代理商。
						 </div>
				  </div>
				  					 
				  <div class="swiper-slide" lang="2">
				  	<div class="cjimg">
						<img alt="河流与绿色森林航拍景观" src="/images/about/up_0492b0a599bdc5dfaa45769fd842b977.webp">
					</div>
				   		 <Div class="qa_7_1_left_1">
						 	无卤						 </Div>
						 <div class="qa_7_1_left_2">
						 	无卤（Halogen-Free）指的是材料中不含或仅含有极低水平的卤素元素（氟、氯、溴、碘），卤素化合物在燃烧时会释放有毒气体，对环境和人体健康构成威胁。宏太正在超越RoHS合规要求，在产品制造过程中采用无卤材料并积极寻找无卤替代材料，加大无卤原辅材料使用，以减少燃烧时可能产生的有毒气体排放，保护环境和公共健康。
						 </div>
				  </div>
				   
		  
				  				  
				  
				</div>
				
				 <div>
				 	<Div class="about_4_1_left_1_left css3" id="l1">
					
					</Div>
					<div class="about_4_1_left_1_right css3" id="r1">
					
					</div>
				 </div>
				
			  </div>

 

		  <!-- Initialize Swiper -->
		  
				
			</div>
			<div class="qa_7_1_right">
								<Div class="css3 qa_7_1_right_bg on" style="background-image:url(/images/about/up_4343f2f6b506c7d16d7eb8a25d9d3569.webp);">
				
				</Div>
								<Div class="css3 qa_7_1_right_bg" style="background-image:url(/images/about/up_7d0bfe246419c5d58653bbe289701260.webp);">
				
				</Div>
								<Div class="css3 qa_7_1_right_bg" style="background-image:url(/images/about/up_0492b0a599bdc5dfaa45769fd842b977.webp);">
				
				</Div>
							</div>
		</div>
	</Div>
</div>
<Div class="clearfix daxie" id="footer">
